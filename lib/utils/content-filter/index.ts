 ...
 * Enterprise-grade content safety for -wagTV
 */
...
export class ContentFilter {
  ...
  private getSaferAlternatives(word: string): string[] {
    // Using a more direct approach with a switch statement to avoid object injection issues
    switch (word) {
      case 'violent':
        return ['energetic', 'dynamic', 'action-packed'];
      case 'scary':
        return ['mysterious', 'intriguing', 'suspenseful'];
      case 'dark':
        return ['nighttime', 'shadowy', 'dimly lit'];
      case 'fight':
        return ['competition', 'contest', 'challenge'];
      case 'weapon':
        return ['tool', 'equipment', 'device'];
      case 'blood':
        return ['red liquid', 'paint', 'ketchup'];
      case 'dead':
        return ['still', 'motionless', 'sleeping'];
      case 'kill':
        return ['stop', 'end', 'finish'];
      default:
        return ['Consider a different approach', 'Try more positive language'];
    }
  }
