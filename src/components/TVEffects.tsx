...
export const TVEffects: React.FC<TVEffectsProps> = ({ effect, onEffectComplete, audioEnabled = true }) => {
  ...
  const renderColorBars = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    const colors = ['#C0C0C0', '#C0C000', '#00C0C0', '#00C000', '#C000C0', '#C00000', '#0000C0', '#000000'];
  ...
  const renderCrashEffect = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    ...
    const animate = () => {
      ...
      ctx.textAlign = 'center';
      if (frame % 20 < 10) ctx.fillText('SIGNAL LOST', canvas.width / 2, canvas.height / 2);
